...
- [x] **C02: std contract + rewrite**
	...
	- **DONE**: API 用 method + pipe-friendly free function 组合；复杂 builder 可以内部用 `Ref`，但 mutable 能力必须在类型上可见并可被 usage/escape 分析。
...
- [x] **C04: regex rewrite in level-1b**
	- **DONE**: 用 level-1b + std/prelude 重写 regex AST/parser/compiler/matcher，支持 UTF-8/WTF-8 byte model、Unicode-aware character classes、lookahead、lookbehind、lookaround、longest-match 所需能力。
...
- [x] **C05: wasm chibalex rewrite**
	- **DONE**: 用 level-1b regex 编写新的 chibalex，包括 `.chibalex` parser、lexer IR、mode/state、longest-match、string/raw-string mode、token codegen。
...
- [x] **C06: wasm chibacc rewrite**
	- **DONE**: 用 level-1b 编写新的 chibacc，包括 `.chibacc` meta-parser、grammar IR、Pratt table、recovery IR、parser codegen。
