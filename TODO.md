...
- [ ] level-1b 不可用状态修复 / truthfulness cleanup
	...
	- **第一轮 audit 清单**:
		...
		- C07 已新增 byte-level source scanner primitives：`source_starts_with_at`、line advance、first namespace scan、namespace name slice；`ProjectSurface` 现在携带 `SourceProjectFacts`，可由 `scan_project_source_facts` 从已加载 files 派生 header facts（`#![Metal]` / `#![no_prelude_import]`）与 namespace scan facts。这是 source facts primary path 的起点，不是 full parser，仍需接入 project load / doc / compile_if / item scan 后才能解除 parser execution blocker。
