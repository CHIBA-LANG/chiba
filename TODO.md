...
- [ ] level-1b 不可用状态修复 / truthfulness cleanup
	...
	- **第一轮 audit 清单**:
		...
		- C07 source semantic gate 现在读取 `ProjectSurface.facts.namespaces`：无 namespace 报 `missing-facts: namespace scan found no namespace`，有 namespace 后仍 fail-closed 于 `missing-facts: source item scan absent`，不再使用固定 dummy blocker。
