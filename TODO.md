...
- [ ] level-1b 不可用状态修复 / truthfulness cleanup
	...
	- **第一轮 audit 清单**:
		...
		- C07 已新增 byte-level source scanner primitives：`source_starts_with_at`、line advance、first namespace scan、namespace name slice；`ProjectSurface` 现在携带 `SourceProjectFacts`，可由 `scan_project_source_facts` 从已加载 files 派生 header facts（`#![Metal]` / `#![no_prelude_import]`）、doc presence facts（`///` / `#[doc(path=...)]`）、`compile_if` expression slice/classification facts 与 namespace scan facts。这是 source facts primary path 的起点，不是 full parser，仍需接入 project load / compile_if predicate parse / item scan 后才能解除 parser execution blocker。
		...
		- C05 chibalex contract 已固定 UTF-8/XID identifier policy，engine state advance 改为 codepoint offset (`next_char_offset`)；`utf8-ident.chibalex` mini fixture 覆盖 `$XID_START/$XID_CONTINUE`。真实 generated lexer runner 仍 blocked 于 level-1b primary compiler execution。
		- C05/C06 generated lexer/parser codegen artifacts 现在带 `ContractOnly` status 与 `missing-lowering` diagnostic，避免 header/comment-only source 被误读为 executable codegen complete。
