...
- [ ] **Pre-C03: L2 type/method/row semantic implementation**
	...
	- **PROGRESS**: `level1c.o check` 已从 source semantic gate 迁到 L2 typed semantic pass，先按 AST/L2Module 检查 Ref/Atomic capability、Ptr/UnsafeRef unsafe API 使用、Metal raw i64 pointer API、method resolution 三路径和 row field obligation；`vp run semantic:gates` 与 `vp run level1b:capability` 已覆盖 valid/invalid fixtures。已新增最小 HM surface gate：函数返回类型可省略，普通函数参数/let 可省略并由使用点或返回表达式形成基础推导；显式 generic 函数仍要求参数/返回类型标注。基础 kind/unify gate 已进入 L2 check，覆盖显式返回类型、typed let、二元表达式的 i64/bool mismatch。extern ABI typing 已进入 L2 check：支持 `wasi`/`C`/`c`/`env`、要求显式 extern 签名，并固定首个 `wasi.fd_write` 签名 gate。namespace 多文件 gate 已有 compiler-side `level1c.o check-project`，能按确定性文件集检查同 namespace fragment 与 consumer，并拒绝缺失 fragment。nominal row/data/union well-formedness 已进入 L2 check，覆盖 type duplicate field、data-record duplicate field、union duplicate field 和 valid fixture；当前 parser 对 ADT variant fixture 仍不稳定，variant duplicate gate 暂留在 pass 逻辑中但未作为 fixture 验收。完整 let-generalization、完整 row unify 和通用 project scanner 仍未完成。
