...
- [x] **Pre-C03: L2 type/method/row semantic implementation**
	...
	- **DONE**: `level1c.o check` 已从 source semantic gate 迁到 L2 typed semantic pass / L2 AST side-table。已覆盖 HM surface、显式 generic、unification、row shorthand、row-field constraint、checked-template row-bound instantiation、method 三路径与 missing method、namespace 多文件、Ref/UnsafeRef/Ptr/Atomic capability、unsafe boundary、Metal raw pointer audit、extern ABI、record duplicate、nominal/data/union duplicate。验证命令：`vp run smoke:bootstrap`、`vp run semantic:gates`、`vp run level1b:type-system`、`vp run level1b:capability`、`vp run run:all-wat`。
...
- [ ] **Pre-C06: wasm-gc CoreIR + validator**
	...
	- **PROGRESS**: L7 Core pass 已递归给表达式节点打 `L7CoreOp`，L8 validator 保留 L7 payload 并递归输出 `L8ValidatedCoreOp`；`vp run smoke:bootstrap` 覆盖 `core-op string-slice`、`core-op continuation-package`、validation ok，以及 `core-invalid-smoke` 的 validator error dump。validator gate 已能拒绝 missing L7 core op、dangling unresolved sentinel、错误 String/str/Slice payload、非法 non-many continuation package、非法 tailcall payload；tail-position call 会 dump 为 `core-op tailcall`；合法 continuation 与 string/slice nanopass 不产生 validation error。仍缺真实 symbol table/layout ref、world/thread validator 和 emitter 改道 validated Core。
	- **TEST**: `timeout 10 ./chibac_amd64-unknown-linux_chiba_dev.o --project . --entry chiba_level1c_main.chiba --output level1c.o` 确认 phase1/resolve 后在后续阶段超时；`timeout 120 ...` 成功写出 `target/debug/level1c.o`；`timeout 10 ./target/debug/level1c.o core-invalid-smoke chiba-level1-grammar-spec/01-test.chiba`；`timeout 10 ./target/debug/level1c.o nanopass supports/bootstrap/wat-tailcall-smoke.chiba | rg -n "core-op tailcall|validation err"` 只匹配 `core-op tailcall`；`timeout 10 ./target/debug/level1c.o nanopass supports/bootstrap/continuation-multi-resume.chiba | rg -n "validation err"` 无匹配；`timeout 120 vp run smoke:bootstrap`、`semantic:gates`、`level1b:type-system`、`run:all-wat`；all-wat `executed=38 instantiated=27`。
	- **HASH**: seed `7a7744ab9ace3d8e13ede45f2e5978e56cc07f597884085a9c4886753f3e268d`；`target/debug/level1c.o` `cb7419a05956613431a29d1af1ff36486aefb3b6ae7fef41d03252e6588c7e01`；`ir.chiba` `993bf1fe9933ca0eeff8dac6c3199ca4c9ec1ae855114746e5840d723b2ff7cc`；`core.chiba` `7fb7b99fbb96f49896ae59188e35ecee487575247fa3fab2b1b9b866b528f6a4`；`show.chiba` `e439b34e520b5af2e544a89ded7cb4923dee7fab3cf81c8371ddb622453648d2`；`validate_core.chiba` `ea8aec619ca84b75244235962473f75fd5c087281785e6820d8b2e0d24be8bb5`；`chiba_level1c_main.chiba` `c1cbc5f5f7a46d99d209175c505e1b51c664f4875b1b7e4192e6959fd60f33dc`；`run-bootstrap-smokes.mjs` `13347840ad1235cb2415a10327487633151ab25c7857a2c1b67a5b59b80c70a0`。
