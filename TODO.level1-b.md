...
- [ ] one-pass CPS + beta：level0 已有真实 one-pass CPS 参考；level-1b 还没完成。当前 level-1b 只是事实流/窄 typed term threading，不能算完成；`src/backend/cir/cps.chiba` 也只是 wrapper/synthetic continuation package，不是可迁移的真实算法。
	...
	- [ ] 算法核心签名必须是 `T(expr, k_meta)` / `lower_expr(expr, k_meta, ...)`，其中 `k_meta: Val -> CpsExpr` 是编译器内部函数；object-level `lambda/app` 才会进入 CPS IR，meta-level `lambda/app` 必须在 pass 执行时通过宿主语言调用和 beta 归约消失。这里的 `lambda`/`app` 必须分两层建模：object-level 是输出 IR，meta-level 是实现 one-pass lowering 的宿主函数调用，不能混成同一种 CPS AST 节点。
