...
- [ ] chibalex/chibacc frontend：chibalex token primary path 已开始替换 scanner fallback；chibacc mini AST evidence 已进入 C07 fact shape，完整 chiba-level1 source AST 仍未接管 primary path。
	...
	- [x] source gate AST-primary item priority：C07 在 `ast_primary_path_blocked == false` 且存在 parser-owned AST item 时，不再让 scanner-derived compile_if/body/extern/Self/duplicate item checks 阻断主线；这些 scanner checks 只在 fallback path 生效。
...
- [ ] one-pass CPS + beta：level0 已有真实 one-pass CPS 参考；level-1b 还没完成。当前 level-1b 只是事实流/窄 typed term threading，不能算完成；`src/backend/cir/cps.chiba` 也只是 wrapper/synthetic continuation package，不是可迁移的真实算法。
	...
	- [ ] 算法核心签名必须是 `T(expr, k_meta)` / `lower_expr(expr, k_meta, ...)`，其中 `k_meta: Val -> CpsExpr` 是编译器内部函数；object-level `lambda/app` 才会进入 CPS IR，meta-level `lambda/app` 必须在 pass 执行时通过宿主语言调用和 beta 归约消失。
	...
	- [ ] `f(x)` 的推导必须在编译期完成两次 meta-level beta：`T(f, fn f2 => T(x, fn v => f2(v, lambda w => k_meta(w))))` 直接输出真实 call，不允许留下 `lambda f2` / `lambda v` 这种 administrative object-level 节点。
